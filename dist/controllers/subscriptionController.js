...
export const subscribeToPlan = async (req, res) => {
    ...
    if (!planId || !user?.email || !user?.id) {
        return errorHandler({ status: 400, message: "Plan ID and user email required" }, req, res, () => { });
    }
    const plan = await getPlanById(planId);
    if (!plan || !plan.paystackPlanCode) {
        return errorHandler({ status: 404, message: "Plan not found" }, req, res, () => { });
    }
    // Calculate subscription end date
    const durationMs = plan.interval === "monthly"
        ? 30 * 24 * 60 * 60 * 1000
    ...
    try {
        ...
        return errorHandler({ status: 500, message: error.message || "Failed to create subscription" }, req, res, () => { });
    }
};
export const getSubscriptionInfo = async (req, res) => {
    ...
    try {
        ...
        return errorHandler({ status: 500, message: error.message || "Failed to fetch subscription info" }, req, res, () => { });
    }
};
export const getAvailablePlans = async (req, res) => {
    try {
        ...
        return errorHandler({ status: 500, message: error.message || "Failed to fetch available plans" }, req, res, () => { });
    }
};
export const cancelUserSubscription = async (req, res) => {
    ...
    try {
        ...
        return errorHandler({ status: 500, message: error.message || "Failed to cancel subscription" }, req, res, () => { });
    }
};
