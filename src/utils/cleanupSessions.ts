...
export const cleanupIdleSessions = async (maxIdleMinutes: number = 30): Promise<number> => {
    try {
        ...
        const deletePromises = idleSessions.map(session =>
            endSession(`${session.userId}_${session.deviceId}`)
        );
